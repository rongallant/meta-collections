...
public function __construct(){
	...
	*/
	...
	add_action('wp_ajax_metafieldorder', array($this, 'userinterface'));
...
/**
    ...
    *
    * @access public
    * @todo in the meatadafield option an order should be stored. This way the user can appoint their own order for the columns.
    */
...
$metadataset = get_option("metadata_".$post_type); //{}
		...
		if(is_array($metadataset)){
            foreach($metadataset as $metafieldID=>$metadata){ 
	        //print_r($metadata[overview]);	            
            if($metadata[overview]==1){
	         $new_columns[$metafieldID] =  $metadata[label];
            }
...
/**
    ...
    *
    * @access public
    * @todo build a switch for getting the right value displayed at the proper wat at the proper place.
    */
...
//print_r($metadataset[$column_name]['type']);
...
if($args[active]=="1"){
  ...
  $labels= preg_replace("/%s/", $labels[name],  $labels);
  $labels= preg_replace("/%n/", $labels[singular_name],  $labels);
	...
	if(is_array($this->metadataset)){
		...
		if($has_taxonomies){ //if there are resistered taxonomy fields loop trough them
			...
			foreach($this->metadataset as $metadataID=>$metaINFO){ 
			    ...
			    'search_items' =>  __('Search')." ".$metaINFO[label],
			    'all_items' => __( 'All')." ".$metaINFO[label],
			    'parent_item' => __( 'Parent ')." ".$metaINFO[singular_name],
			    'parent_item_colon' => __( 'Parent')." ".$metaINFO[singular_name],
			    'edit_item' => __( 'Edit')." ".$metaINFO[label], 
			    'update_item' => __( 'Update')." ".$metaINFO[label],
			    'add_new_item' => __( 'Add New', "_coll" )." ".$metaINFO[label],
			    'new_item_name' => __( 'New Name:')." ".$metaINFO[label],
			    'menu_name' => $metaINFO[label]
			  ); 	
...
/**
    ...
    * Single action
    * @access public
    * @todo properly handle autosave request for saving post meta
    */
