...
function showfield($post=null, $element=null, $c=null){
			...
			$map 		= "map_{$element[ID]}";
			$addressval	= ($value[address]=="") ? $element[default_value] : $value[address];
			$latval		= ($value[latitude]=="")? $element[latitude]: $value[latitude];
			$longval	= ($value[longitude]=="")? $element[longitude]: $value[longitude];
...
echo"<script>";
	...
	$zoom 		= ($element[zoom]=="")? 10 : $element[zoom];
	 ...
	 jQuery(document).ready(function () {
	 		...
	 		placeMarks = [{title: 'testmarker', latitude: '{$latval}', longitude: '{$longval}'}];
	 		{$map} = googleMaps;
	        mapOptions = {zoom: {$zoom},center: new google.maps.LatLng('{$element[latitude]}', '{$element[longitude]}'), mapTypeId: google.maps.MapTypeId.ROADMAP};
